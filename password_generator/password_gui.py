...
def get_pw_options():
    ...
    is_valid, special_error_message, special_characters, required_specials = password_tools.validate_specials(
        entered_specials)
...
def generate_password(keypress=None):
    ...
    if length < min_length:
        status_label.config(
            text=f'Password must be at least {min_length} with current options.\n', foreground='red')
        generated_pw.config(text='')
        return

    password = password_tools.generate_pw(
        length, numbers, special_characters, required_specials)
...
def copy_password(keypress=None):
    ...
    status_label.config(
        text='Password copied to clipboard!', foreground='blue')


def show_specials_help(event=None):
    status_label.config(
        text="Empty = all punctuation, none = no specials, or type allowed specials like !@#", foreground='gray')


def show_password_help(event=None):
    status_label.config(
        text="Generated password. Click Copy or press Ctrl+C.", foreground='gray')


def show_generate_help(event=None):
    status_label.config(
        text='Press "Enter" at any time to generate password. \n default fields produce a unique string with 2 of each character type.', foreground='gray')
...
length_label = tk.Label(form_frame2, text='Length ',
...
specials_label = tk.Label(
    form_frame2, text='Special characters ', background='gray', foreground='white')
...
numbers_label = tk.Label(form_frame2, text='Use numbers ',
                         background='gray', foreground='white')
numbers_label.grid(row=2, column=0, sticky='e')
numbers_check = tk.Checkbutton(form_frame2, variable=numbers_var,
                               justify='left', background='gray', highlightbackground='gray')
numbers_check.grid(row=2, column=1, sticky='w', pady=10)

generate_button = tk.Button(
    form_frame2, text='Generate', command=generate_password,)
...
soc_label = tk.Label(form_frame2, text='Save settings on close ',
                     background='gray', foreground='white')
soc_label.grid(row=2, column=2, sticky='e')
soc_check = tk.Checkbutton(
    form_frame2, variable=soc_var, background='gray', highlightbackground='gray')
...
generated_pw = tk.Label(form_frame3, text='', width=50,
